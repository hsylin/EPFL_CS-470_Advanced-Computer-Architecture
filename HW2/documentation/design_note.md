...
Input `nop` instructions are treated as semantic no-ops and are removed before dependency analysis and scheduling. Since they do not affect the architectural state, they should not constrain the scheduler.

In a statically scheduled processor, `nop` instructions are used to create bubbles that satisfy timing constraints, such as RAW dependencies, or to fill unused execution-unit slots in a VLIW bundle. However, because this project implements the scheduler itself, these bubbles should be generated by the scheduler rather than preserved from the input program. Therefore, input `nop` instructions are discarded, and output `nop` instructions are inserted only when needed to represent empty bundle slots or required delay cycles.
