	...
	const basic_block_info_t& block_info)
{
    ...
    for (int i = 0; i < size_of_block_0; i++)
    {
        ...
        for (dependency_t dependency : dependency_table.entries[i].local_dependencies) {
            scheduled_slot = max(scheduled_slot, find_earliest_satisfying_cycle(program, dependency));
        }
    ...
    if (block_info.has_loop) {
        ...
        for (int i = block_info.loop_start_address; i < block_info.loop_instruction_address; i++) {
            ...
            for (dependency_t dependency : dependency_table.entries[i].loop_invariant_dependencies) {
                bb0_to_bb1_padding = max(
                    bb0_to_bb1_padding,
                    find_earliest_satisfying_cycle(program, dependency) - schedule.size()
                );
            }
            ...
            for (dependency_t dependency : dependency_table.entries[i].interloop_dependencies) {
                initiation_interval = max(initiation_interval, find_ii_satisfying_interloop(program, dependency));
            }
        }
        ...
        for (int i = block_info.loop_instruction_address + 1; i < static_cast<int>(program.size()); i++)
        {
            ...
            for (dependency_t dependency : dependency_table.entries[i].post_loop_dependencies) {
                scheduled_slot = max(scheduled_slot, find_earliest_satisfying_cycle(program, dependency) - schedule.size());
            }

            scheduled_slot = find_valid_cycle(bb2_schedule, scheduled_slot, program[i].unit);
            schedule_instruction(bb2_schedule, scheduled_slot, program[i]);
        }
